...
class IndexTTS:
    ...
    def _get_conditioning_mel(self, audio_prompt, verbose=False):
        ...
        return self.cache_cond_mel

    def _preprocess_text(self, text, max_text_tokens_per_sentence, verbose=False):
    ...
    def _generate_speech_codes(self, auto_conditioning, batch_text_tokens, cond_mel_lengths, **generation_kwargs):
        with torch.no_grad():
    ...
    def _generate_latents(self, auto_conditioning, text_tokens, codes, code_lens):
        with torch.no_grad():
    ...
    def _vocode_latents(self, latents, auto_conditioning):
        with torch.no_grad():
            with torch.amp.autocast(latents.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                wav, _ = self.bigvgan(latents, auto_conditioning.transpose(1, 2))
        return wav

    def _save_wav(self, wav, output_path, sampling_rate):
        wav = wav.cpu()  # to cpu
        if output_path:
            # 直接保存音频到指定路径中
    ...
    def infer_fast(self, request: TTSRequest):
        """
        Args:
        ...
        sentences = self._preprocess_text(text, config.max_text_tokens_per_sentence, verbose=verbose)
        wavs = []
        gpt_gen_time = 0
        gpt_forward_time = 0
        bigvgan_time = 0
        ...
        bucket_max_size = config.sentences_bucket_max_size if self.device != "cpu" else 1
        ...
        all_latents, gpt_gen_time, gpt_forward_time = self._process_buckets_fast(all_sentences, auto_conditioning, cond_mel_lengths, verbose, **config.__dict__)
        # bigvgan chunk
        chunk_size = 2
        ...
        for items in chunk_latents:
            ...
            wav = self._vocode_latents(latent, auto_conditioning)
        ...
        return self._save_wav(wav, output_path, sampling_rate)

    def _process_buckets_fast(self, all_sentences, auto_conditioning, cond_mel_lengths, verbose=False, **generation_kwargs):
        ...
        for item_tokens in all_text_tokens:
            ...
            m_start_time = time.perf_counter()
            temp_codes = self._generate_speech_codes(auto_conditioning, batch_text_tokens, cond_mel_lengths,
                                                     **generation_kwargs)
        ...
        for batch_codes, batch_tokens, batch_sentences in zip(all_batch_codes, all_text_tokens, all_sentences):
            for i in range(batch_codes.shape[0]):
                ...
                m_start_time = time.perf_counter()
                latent = self._generate_latents(auto_conditioning, text_tokens, codes, code_lens)
    ...
    def infer(self, request: TTSRequest):
        ...
        for sent in sentences:
            ...
            m_start_time = time.perf_counter()
            codes = self._generate_speech_codes(auto_conditioning, text_tokens,
...
if __name__ == "__main__":
    ...
    request = TTSRequest(text=text, audio_prompt=prompt_wav, output_path="gen.wav", verbose=True)
    tts.infer(request)
